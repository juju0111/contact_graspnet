...
def get_model(point_cloud, is_training, global_config, bn_decay=None):
    ...
    if (
        ...
        point_cloud = gather_point(
            point_cloud, farthest_point_sample(data_config["ndataset_points"], point_cloud)
        )
    ...
    if joint_heads:
        ...
        approach_dir_head_orthog = tf.math.l2_normalize(
            approach_dir_head
            - tf.reduce_sum(
                tf.multiply(grasp_dir_head_normed, approach_dir_head), axis=2, keepdims=True
            )
    ...
    end_points["grasp_offset_pred"] = (
        tf.math.sigmoid(grasp_offset_head) if model_config["bin_offsets"] else grasp_offset_head
    )
...
def build_6d_grasp(
    ...
    # def build_6d_grasp(
    #     approach_dirs, base_dirs, contact_pts, thickness, use_tf=False, gripper_depth=0.2075
    # ):
    """
    ...
    Keyword Arguments:
        ...
        gripper_depth {float} -- distance from gripper coordinate frame to gripper baseline in m (default: {0.1034})
                                                                                                (change default for robotiq-2f_85 model)
    ...
    if use_tf:
        grasps_R = tf.stack(
            [base_dirs, tf.linalg.cross(approach_dirs, base_dirs), approach_dirs], axis=3
        )
        grasps_t = (
            contact_pts
        ...
        for i in range(len(contact_pts)):
            ...
            grasp[:3, 3] = (
                contact_pts[i] + thickness[i] / 2 * grasp[:3, 0] - gripper_depth * grasp[:3, 2]
            )
...
def get_losses(
    ...
    if global_config["MODEL"]["bin_offsets"]:
        ...
        thickness_gt = tf.gather_nd(
            get_bin_vals(global_config), tf.expand_dims(tf.argmax(offset_labels_pc, axis=2), axis=2)
        )
    else:
    ...
    gt_grasps_proj = build_6d_grasp(
        approach_labels_pc_cam, dir_labels_pc_cam, pointclouds_pl, thickness_gt, use_tf=True
    )  # b x num_point x 4 x 4
    pos_gt_grasps_proj = tf.where(
        tf.broadcast_to(
            tf.expand_dims(tf.expand_dims(tf.cast(grasp_success_labels_pc, tf.bool), 2), 3),
    ...
    gripper_control_points_homog = tf.concat(
        [
            ...
            tf.ones((global_config["OPTIMIZER"]["batch_size"], gripper_control_points.shape[1], 1)),
        ],
        axis=2,
    )  # b x 5 x 4
    ...
    sym_control_points = tf.keras.backend.repeat_elements(
        tf.expand_dims(sym_gripper_control_points_homog, 1), gt_grasps_proj.shape[1], axis=1
    )  # b x num_point x 5 x 4
    pred_control_points = tf.matmul(control_points, tf.transpose(pred_grasps, perm=[0, 1, 3, 2]))[
        :, :, :, :3
    ...
    squared_add = tf.reduce_sum(
        (tf.expand_dims(pred_control_points, 2) - tf.expand_dims(gt_control_points, 1)) ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    sym_squared_add = tf.reduce_sum(
        (tf.expand_dims(pred_control_points, 2) - tf.expand_dims(sym_gt_control_points, 1)) ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    ...
    gt_control_points = tf.matmul(control_points, tf.transpose(gt_grasps_proj, perm=[0, 1, 3, 2]))[
        :, :, :, :3
    ]  #  b x num_pos_grasp_point x 5 x 3
    sym_gt_control_points = tf.matmul(
    ...
    neg_squared_adds = -tf.reduce_sum(
        (tf.expand_dims(pred_control_points, 1) - tf.expand_dims(gt_control_points, 2)) ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    neg_squared_adds_sym = -tf.reduce_sum(
        (tf.expand_dims(pred_control_points, 1) - tf.expand_dims(sym_gt_control_points, 2)) ** 2,
        axis=(3, 4),
    )  # b x num_point x num_pos_grasp_point x ( 5 x 3)
    ...
    batch_idcs = tf.meshgrid(
        tf.range(pred_grasp_idcs_joined.shape[1]), tf.range(pred_grasp_idcs_joined.shape[0])
    )
    ...
    approach_labels_orthog = tf.math.l2_normalize(
        approach_labels_pc_cam
        - tf.reduce_sum(tf.multiply(grasp_dir_head, approach_labels_pc_cam), axis=2, keepdims=True)
        * grasp_dir_head,
        axis=2,
    )
    ...
    bin_ce_loss = tf.nn.sigmoid_cross_entropy_with_logits(
        labels=tf.expand_dims(grasp_success_labels_pc, axis=2), logits=end_points["binary_seg_head"]
    )
...
def compute_labels(
    ...
    contact_point_dirs_batch_cam = tf.matmul(
        contact_point_dirs_batch, tf.transpose(camera_pose_pl[:, :3, :3], perm=[0, 2, 1])
    )[:, :, :3]
    ...
    pos_contact_approaches_batch_cam = tf.matmul(
        pos_contact_approaches_batch, tf.transpose(camera_pose_pl[:, :3, :3], perm=[0, 2, 1])
    )[:, :, :3]
    ...
    if filter_z:
        dir_filter_passed = tf.keras.backend.repeat_elements(
            tf.math.greater(contact_point_dirs_batch_cam[:, :, 2:3], tf.constant([z_val])),
            3,
            axis=2,
        )
    ...
    squared_dists_all = tf.reduce_sum(
        (tf.expand_dims(contact_point_batch_cam, 1) - tf.expand_dims(xyz_cam, 2)) ** 2, axis=3
    )
