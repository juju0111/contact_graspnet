...
class GraspEstimator:
    ...
    def build_network(self, robot):
        ...
        offset_bin_pred_vals = (
            tf.gather_nd(
                tf_bin_vals,
                tf.expand_dims(tf.argmax(end_points["grasp_offset_head"], axis=2), axis=2),
            )
            ...
            for k in segment_keys:
                ...
                if np.any(pc_segments[k]) and np.any(contact_pts[j]):
                    ...
                    if local_regions and np.any(pred_grasps_cam[k]):
                        print(
                            "Generated {} grasps for object {}".format(len(pred_grasps_cam[k]), k)
                        )
                else:
                    print(
